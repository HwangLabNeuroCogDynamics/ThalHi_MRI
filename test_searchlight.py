...
subject = input()
#subject='10011'
func_path = "/Shared/lss_kahwang_hpc/data/ThalHi/3dDeconvolve/sub-%s/" %subject
...
searchlight_context = nilearn.decoding.SearchLight(mask, process_mask_img=mask, estimator = pipeline, radius=6, n_jobs=4, verbose=0, cv=LeaveOneGroupOut())
...
searchlight_color = nilearn.decoding.SearchLight(mask, process_mask_img=mask, estimator = pipeline, radius=6, n_jobs=4, verbose=0, cv=LeaveOneGroupOut())
...
searchlight_shape = nilearn.decoding.SearchLight(mask, process_mask_img=mask, estimator = pipeline, radius=6, n_jobs=4, verbose=0, cv=LeaveOneGroupOut())
...
searchlight_task = nilearn.decoding.SearchLight(mask, process_mask_img=mask, estimator = pipeline, radius=6, n_jobs=4, verbose=0, cv=LeaveOneGroupOut())
